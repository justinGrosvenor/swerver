...
pub const Handler = struct {
    ...
    pub fn processPacket(
        ...
        // Helper: clear any stale h3 events on the connection so this
        // packet only ever sees events generated by its own frames.
        const clearEvents = struct {
            fn call(c: *connection.Connection) void {
                if (c.http3_stack) |*stack| stack.events.clearRetainingCapacity();
            }
        }.call;
    ...
    fn processStreamFrame(conn: *connection.Connection, stream_frame: frame.StreamFrame) Error!void {
        ...
        // Snapshot recv_offset before delivery so we can detect whether
        // this STREAM frame actually advanced the stream (vs. being a
        // duplicate/old retransmit silently dropped by stream.receive).
        ...
        // If recv_offset didn't advance, this frame was a duplicate or
        // pure out-of-order data with no in-order progress. Don't fire
        // HTTP/3 events for it — that would re-route the same request
        // to the application layer and produce duplicate responses.
        if (s.recv_offset == recv_offset_before) {
            return;
        }

        // Update connection-level flow control by the bytes that actually
        // landed (recv_offset_before → s.recv_offset).
        const new_bytes = s.recv_offset - recv_offset_before;
        conn.flow_control.onDataReceived(new_bytes) catch {
            return Error.ProtocolError;
        };

        // Process through HTTP/3 stack — feed the bytes that just landed,
        // not the full frame data (which may overlap older data already
        // delivered to the h3 stack).
        // Note: this currently re-feeds stream_frame.data because the
        // stream layer doesn't expose the newly-appended slice. The
        // duplicate-frame guard above prevents the worst case (re-feeding
        // an entire request); fully avoiding partial overlap is a
        // follow-up.
        const fin_now = stream_frame.fin or
            (s.recv_fin_offset != null and s.recv_offset >= s.recv_fin_offset.?);
        _ = conn.processHttp3Stream(stream_frame.stream_id, stream_frame.data, fin_now) catch |err| {
