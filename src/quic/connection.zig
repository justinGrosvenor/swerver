...
pub const Connection = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, is_server: bool, dcid: types.ConnectionId) Connection {
        ...
        if (RAND_bytes(&conn.our_cid.bytes, 8) != 1) {
            var rng = std.Random.DefaultPrng.init(
                (if (clock.Instant.now()) |i| i.ns else 0) ^ std.hash.Wyhash.hash(0, dcid.slice()),
            );
    ...
    pub fn startPathValidation(self: *Connection) [8]u8 {
        ...
        if (RAND_bytes(&data, 8) != 1) {
            var prng = std.Random.DefaultPrng.init(if (clock.Instant.now()) |i| i.ns else 42);
