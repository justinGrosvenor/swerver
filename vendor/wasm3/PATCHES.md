...
for swerver's WASM edge functions (design 10.0). The build compiles the 13 core
translation units (see `build.zig`, `WASM3_FILES`). The WASI/tracer/meta API
bindings are not vendored at all: a sandboxed filter has no ambient syscall
surface, and shipping unused WASI source would contradict that. Specifically
DROPPED from the upstream `source/` tree: `m3_api_uvwasi.c` (needs the external
uvwasi lib), `m3_api_wasi.c` + `m3_api_wasi.h`, `m3_api_meta_wasi.c`, and
`m3_api_tracer.c` + `m3_api_tracer.h`. None are referenced by the compiled
units.
