...
// --- swerver FUEL PATCH (design 10.0 resource-bounding) ---------------------
// Upstream wasm3 has no fuel/op-count metering. This is the minimal localized
// patch that charges one unit per loop back-edge (see op_Loop in m3_exec.h),
// trapping when exhausted, so a runaway guest filter is interrupted instead of
// wedging the single-threaded worker. Process-global is correct: swerver runs
// one single-threaded worker per process. Externs in m3_fuel.h.
int64_t m3_swerver_fuel = INT64_MAX;
const char* const m3Err_trapFuel = "[trap] fuel exhausted";
// ----------------------------------------------------------------------------
