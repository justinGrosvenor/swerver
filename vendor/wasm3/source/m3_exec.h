...
d_m3Op  (Loop)
{
    ...
    do
    {
        // swerver FUEL PATCH: charge one unit per loop iteration; trap on empty.
        // This is the back-edge of every wasm loop, so a runaway while(1) is
        // interrupted here even when it never calls an import. See m3_fuel.h.
