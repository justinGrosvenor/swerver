...
| `collector_url` | — | OTLP collector endpoint |
| `service_name` | `swerver` | `service.name` resource attribute |
| `sample_rate` | — | Sampling rate, `1`–`10000` (where `10000` = 100%) |
| `flush_interval_s` | — | How often batched spans are flushed |
| `max_batch_size` | — | Max spans per export batch |
