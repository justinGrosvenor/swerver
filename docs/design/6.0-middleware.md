...
- support route-scoped chaining and access to app state/services via handler context.
...
- Middleware responses must use managed buffers (via `middleware.respondManaged`) so body slices outlive the request, with a `503` fallback if buffers are exhausted.
...
## Route-scoped middleware + DI

- Routers can attach middleware per route or per group for localized policy.
- Middleware should read app state or typed services from the handler context (no global singletons).
- Middleware must not allocate in the hot path; state access is by pointer.
