...
Defines the architecture, configuration, and behavior for Swerver acting as a reverse proxy: forwarding client requests to upstream servers and relaying responses back.
...
Retry is configurable per route from the JSON config via `routes[].retry.max_retries`
(`RouteJson.retry` → `ProxyRoute.retry.max_retries`; default `1`). The retryable
status set defaults to `{502, 503, 504}` and the retryable method set to
`{GET, HEAD, OPTIONS}`.
...
On a retryable 5xx status, the upstream connection is released **closed**
(`pool.release(c, now_ms, false)`) rather than kept alive, so the retry opens a
**fresh** connection. Under a SO_REUSEPORT upstream that fresh connection can land
on a different worker than the one that just returned the transient 5xx, which is
the point of retrying. On connection failures (`error.Empty` / `error.ReadFailed`)
the connection is marked failed and removed from the pool before the next attempt.
