...
This spec defines the architecture, invariants, and validation criteria for a cross-platform, production-grade webserver implemented in Zig with minimal dependencies. The server is intended to embed as a library with a public router/server API.
...
- Provide an embedded library API with a public server, router, and middleware surface.
...
flowchart LR
    ...
    Router --> App[Embedded App Handlers]
